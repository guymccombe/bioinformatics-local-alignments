...
def twoColumnSearch(alphabet, scoringMatrix, sequence1, sequence2):
    ...
    for i in range(len(sequence1)):
        ...
        for j in range(1, len(sequence2) + 1):
            upLeft = lastCol[j-1] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, sequence1[i], sequence2[j-1])
            up = column[j-1] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, '-', sequence2[j-1])
            left = lastCol[j] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, sequence1[i], '-')

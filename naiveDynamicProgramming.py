    ...
    for i in range (1, len(alignmentScoreMatrix)):
        for j in range (1, len(alignmentScoreMatrix[0])):
            alignmentScoreMatrix[i][j], alignmentBacktrackMatrix[i][j] = max([
                (alignmentScoreMatrix[i-1][j-1] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, sequence1[i-1], sequence2[j-1]), 'D'),
                (alignmentScoreMatrix[i-1][ j ] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, sequence1[i-1], '_'), 'U'),
                (alignmentScoreMatrix[ i ][j-1] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, '_', sequence2[j-1]), 'L'),
                (0, 'G')
            ])
...
def initialiseMatrices(alphabet, scoringMatrix, sequence1, sequence2):
    ...
    for i in range(1, len (alignmentScore)):
        alignmentScore[i][0], backtrackMatrix[i][0] = max(
            (alignmentScore[i-1][0] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, '_', sequence1[i-1]), 'U'),
            (0, 'G')
        )

    for i in range(1, len (alignmentScore[0])):
        alignmentScore[0][i], backtrackMatrix[0][i] = max (
            (alignmentScore[0][i-1] + getScoreOfMatchingCharactersFromScoringMatrix(alphabet, scoringMatrix, '_', sequence2[i-1]), 'L'),
            (0, 'G')
        )
...
a = dynprog(alphabet, matrix, "DDCDDCCCDCAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACCCCDDDCDADCDCDCDCD", "DDCDDCCCDCBCCCCDDDCDBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBDCDCDCDCD")
print("Score:   ", a[0])
print("Indices: ", a[1],a[2])
